...
(procedure (Print params &tmp hDialog hDText hIcon hEdit btnPressed paramCnt diagX diagY maxWidth newPrintDlg hFirstEnabled oldPort [hButtons 6] btnsWidth buttonCnt btnX btnY [msgBuf 1013] moveToX moveToY)
	(= diagY -1)
	(= diagX -1)
	(= buttonCnt 0)
	(= hEdit NULL)
	...
	(if hIcon
		...
		(hDText moveTo: 5 (+ 10 (hIcon nsBottom?)) )
		(hDialog add: hIcon)

		else
		; original code
		(hIcon moveTo: 4 gIconY)
		(hDText moveTo: (+ 4 (hIcon nsRight?)) 4 )
	...
	(if gVertButtons
		...
		(while (< paramCnt buttonCnt)
			([hButtons paramCnt] moveTo: (+ (hDialog nsLeft?) 4) btnY)
